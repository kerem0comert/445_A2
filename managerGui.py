...
class managerGui():
    ...
    def __init__(self,root):
        ...
        self.femaleVisitors.grid(row=2,column=2,columnspan=10)

        Label(text = ' The Number Of Local Visitors ',font='Times 15').grid(row=4,column=1,pady=10)
        self.localVisitors = Entry()
        self.localVisitors.grid(row=2,column=2,columnspan=10)

        Label(text = ' The Number Of Tourists ',font='Times 15').grid(row=5,column=1,pady=10)
        self.tourists = Entry()
        self.tourists.grid(row=2,column=2,columnspan=10)

        ttk.Button(text='REPORT',command=self.reportData).grid(row=3,column=2)
    ...
    def loginUser(self):
        ...
        if not totVisitors | not maleVisitors | not femaleVisitors | not localVisitors | not tourists: mb.showerror("Error", "Everything has to be filled out!")
        else: 
